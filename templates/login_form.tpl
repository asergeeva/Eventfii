	...
	<div class="section section-login">
      ...
      <p><fb:login-button perms="email,publish_stream" id="fb-login-button" onlogin="FBCON.onlogin()">Login with Facebook</fb:login-button></p>
			...
			<fieldset id="new_user_login_form">
				...
				<label for="ef_login_email_new"><span>Email</span> <input type="text" class="inputbox" name="ef_login_email" id="ef_login_email_new" value="{$smarty.post.email}" /></label>
				<p><span style="color:red; top:0px; font-weight:bold;">{$user_create_email}</span></p>
				
				<label for="ef_login_pass_new"><span>Password</span> <input type="password" class="inputbox" name="ef_login_pass_new" id="ef_login_pass_new" value="{$smarty.post.pass}"/></label>
				<p><span style="color:red; top:0px; font-weight:bold;">{$user_create_pass}</span></p>
				
				<label for="ef_fname_new"><span>First Name</span> <input type="text" class="inputbox" name="ef_fname" id="ef_fname_new" value="{$smarty.post.fname}" /></label>
				<p><span style="color:red; top:0px; font-weight:bold;">{$user_create_fname}</span></p>
				
				<label for="ef_lname_new"><span>Last Name</span> <input type="text" class="inputbox" name="ef_lname" id="ef_lname_new" value="{$smarty.post.lname}" /></label>
				<p><span style="color:red; top:0px; font-weight:bold;">{$user_create_lname}</span></p>
				
				<label for="ef_login_phone_new"><span>Cell Phone #</span> <input type="text" class="inputbox" name="ef_login_phone" id="ef_login_phone_new" value="{$smarty.post.phone}"  /> <p>So you can easily receive event updates through texts!</p></label>
